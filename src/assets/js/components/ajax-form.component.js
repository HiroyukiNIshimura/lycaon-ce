...
parasails.registerComponent('ajaxForm', {
  ...
  mounted: async function () {
    if (this.action === undefined && this.handleSubmitting === undefined) {
      ...
      (!_.isString(this.action) || !_.isFunction(Cloud[_.camelCase(this.action)]))
    ) {
      throw new Error(
    ...
    if (this.handleParsing === undefined && this.formData === undefined) {
      ...
      (!_.isObject(this.formData) || _.isFunction(this.formData) || _.isArray(this.formData))
    ) {
      throw new Error(
  ...
  methods: {
    ...
    _submit: async function () {
      ...
      if (this.handleParsing) {
        ...
        if (Object.keys(formErrors).length > 0) {
          ...
          if (this._environment !== 'production') {
            console.warn(
              `<ajax-form> encountered ${Object.keys(formErrors).length} form error${
      ...
      if (!failedWithCloudExit) {
        ...
        this.$emit('update:cloudErrorRawData', rawErrorFromCloudSDK.responseInfo.data);
        this.$emit('rejected', failedWithCloudExit);
      }
    },
  },
});
