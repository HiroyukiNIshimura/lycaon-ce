module.exports = {
  ...
  exits: {
    success: {
      viewTemplatePath: 'pages/backoffice/organization/edit',
    },
    notFound: {
  ...
  fn: async function (inputs) {
    ...
    response.org = await Organization.findOne({ id: inputs.id });
    if (!response.org) {
      throw 'notFound';
    }

    response.billing = await Billing.findOne({ organization: response.org.id });

    response.prefectures = [
      '',
      '北海道',
      '青森県',
      '岩手県',
