...
(for existing users who just changed their email address).`,
  ...
  fn: async function (inputs) {
    ...
    var user = await User.findOne({ emailProofToken: inputs.token }).populate('organization');
    ...
    if (user.emailStatus === 'unconfirmed') {
      ...
      if ((await User.count({ emailAddress: user.emailChangeCandidate })) > 0) {
        ...
        this.req.session.emailAddressNoLongerAvailable = true;
        throw { redirect: `/${user.organization.handleId}/account` };
      }
